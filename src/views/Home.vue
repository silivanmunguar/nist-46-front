<template>
  <v-container class="fill-height">
    <v-responsive class="d-flex align-center text-center fill-height">
      ...
      <div class="text-left text-h6 pl-2">
        Whitman Repository of Accepted Stability Constants of Aqueous Ligands
      </div>
      <SimpleSearchForm
        v-if="isSimpleSearch"
...
export default defineComponent({
  ...
  data: () => ({
    ...
    ligandCharges: undefined,
    metalCharges: undefined,
    chemicals: undefined,
  }),
  computed: {
  ...
  methods: {
    ...
    searchLigands() {
      ...
      const form = new SearchRequestModel(
        this.ligands,
        this.metals,
        this.categories,
        this.ligandCharges,
      ...
      if (form.onlyHasLigands()) {
        simpleSearch(form.ligands?.split(",") ?? [])
          .then((result) => {
            ...
            store.searchResult =
              result as ProcessedLigandAdvanceSearchResultModel[];
